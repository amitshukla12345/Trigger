...
BEGIN
   ...
   END IF;
   INSERT INTO student_management.teacher_courses
   VALUES(Tname,Cname);
   ...
   */
   ...
   DELIMITER $$
   CREATE TRIGGER before_update_employeee1
   BEFORE UPDATE
   ON employee1 FOR EACH ROW
   BEGIN
      SET NEW.salary=NEW.working_hours*new.pay_per_hour;
   ...
   DELIMITER $$
   CREATE TRIGGER before_update_employee1_pay_per_hour
   Before UPDATE
   ON employee1 FOR Each Row
   BEGIN
      ...
      END$$
      DELIMITER ;
      update employee1 set pay_per_hour=80 where name="Rahul";
       ALTER TABLE employee1 ADD COLUMN d_id varchar(10);
       
       CREATE TABLE department1 (d_id VARCHAR(10),d_name VARCHAR(20));
       SELECT * FROM DEPARTMENT;
       INSERT INTO  department1 VALUES("D1","Marketing"),
		("D2","IT");
        SELECT * FROM DEPARTMENT1;
         update employee1 set d_id="D1"where name="Rahul";
         SELECT * FROM EMPLOYEE1;
         
         DELIMITER $$
         CREATE TRIGGER delete_department1
         BEFORE DELETE
         ON department1 FOR EACH ROW
         BEGIN
         UPDATE employee1 SET d_id=NULL WHERE d_id=old.d_id;
         END$$
         delimiter ;
         
        UPDATE employee1 set d_id=Null where d_id="D1";
